...
/*
	DefineCustomBoolVariable("pg_plan_guarantee.enabled",
							 "Enable the pg_plan_guarantee extension.",
							 NULL,
							 &pgpg_enabled,
							 false,
							 PGC_SUSET,
...
// AST: {QUERY :commandType 1 :querySource 0 :canSetTag true :utilityStmt <> :resultRelation 0 :hasAggs false :hasWindowFuncs false :hasTargetSRFs false :hasSubLinks false :hasDistinctOn false :hasRecursive false :hasModifyingCTE false :hasForUpdate false :hasRowSecurity false :isReturn false :cteList <> :rtable <> :jointree {FROMEXPR :fromlist <> :quals <>} :targetList ({TARGETENTRY :expr {CONST :consttype 25 :consttypmod -1 :constcollid 100 :constlen -1 :constbyval false :constisnull false :location 7 :constvalue 53 [ -44 0 0 0 115 101 108 101 99 116 32 42 32 102 114 111 109 32 112 103 95 99 108 97 115 115 32 119 104 101 114 101 32 114 101 108 110 97 109 101 32 61 32 39 112 103 95 99 108 97 115 115 39 ]} :resno 1 :resname query :ressortgroupref 0 :resorigtbl 0 :resorigcol 0 :resjunk false} {TARGETENTRY :expr {CONST :consttype 25 :consttypmod -1 :constcollid 100 :constlen -1 :constbyval false :constisnull true :location 79 :constvalue <>} :resno 2 :resname use :ressortgroupref 0 :resorigtbl 0 :resorigcol 0 :resjunk false}) :override 0 :onConflict <> :returningList <> :groupClause <> :groupDistinct false :groupingSets <> :havingQual <> :windowClause <> :distinctClause <> :sortClause <> :limitOffset <> :limitCount <> :limitOption 0 :rowMarks <> :setOperations <> :constraintDeps <> :withCheckOptions <> :stmt_location 0 :stmt_len 90}
/*
 parse
	commandType = CMD_SELECT
	utilityStmt = 0x0
	jointree -> fromList = 0x0
	targetList ->
		T_List
		length = 2
		*(TargetEntry*)list_nth(parse->targetList,0)
		  (char*)((TargetEntry*)list_nth(parse->targetList,0))->resname = query
		  *(Const*)((TargetEntry*)list_nth(parse->targetList,0))->expr
		  exprType(... expr) == TEXTOID // same as: ((Const*)((TargetEntry*)list_nth(parse->targetList,0))->expr)->consttype
			TextDatumGetCString
			text_to_cstring((text*)((Const*)((TargetEntry*)list_nth(parse->targetList,0))->expr)->constvalue)
		*(TargetEntry*)list_nth(parse->targetList,1)
		  (char*)((TargetEntry*)list_nth(parse->targetList,1))->resname = plan
	returningList = 0x0
